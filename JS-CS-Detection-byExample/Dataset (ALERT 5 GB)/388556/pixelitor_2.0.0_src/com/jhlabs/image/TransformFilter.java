...
// (int the Filters used by pixelitor)
...
//					rgb[0] = inPixels[i];
    ...
    protected BufferedImage filterPixelsNN(final BufferedImage dst, final int width, int height, final int[] inPixels) {
        ...
        // bizarre, but it seems that it is somewhat faster if the executorService is created every
        // time - even if it is not shut down...
        ...
        for (int y = 0; y < outHeight; y++) {
            ...
            Future<int[]> result = executorService.submit(calculateLineTask);

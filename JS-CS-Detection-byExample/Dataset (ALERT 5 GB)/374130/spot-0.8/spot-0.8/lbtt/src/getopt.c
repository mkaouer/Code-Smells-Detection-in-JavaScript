     ...
     const char *optstring;
{
  ...
  first_nonopt = last_nonopt = gnu_optind;
...
int
_gnu_getopt_internal (argc, argv, optstring, longopts, longind, long_only)
     int argc;
     char *const *argv;
     const char *optstring;
     ...
     int long_only;
{
  ...
  if (gnu_optind == 0 || !__gnu_getopt_initialized)
    {
      if (gnu_optind == 0)
...
# define NONOPTION_P (argv[gnu_optind][0] != '-' || argv[gnu_optind][1] == '\0'	      \
		      || (gnu_optind < nonoption_flags_len			      \
			  && __getopt_nonoption_flags[gnu_optind] == '1'))
#else
# define NONOPTION_P (argv[gnu_optind][0] != '-' || argv[gnu_optind][1] == '\0')
#endif

...
int intf_SysCreate( intf_thread_t *p_intf )
{
    ...
    p_intf->p_vout = vout_CreateThread( main_GetPszVariable( VOUT_DISPLAY_VAR, NULL), 0,
                                        main_GetIntVariable( VOUT_WIDTH_VAR, VOUT_WIDTH_DEFAULT ),
                                        main_GetIntVariable( VOUT_HEIGHT_VAR, VOUT_HEIGHT_DEFAULT ), NULL, 0, NULL );
    ...
    /* Get display pointer */
    p_intf->p_sys->p_display = vout_SysGetVisual( p_intf->p_vout );

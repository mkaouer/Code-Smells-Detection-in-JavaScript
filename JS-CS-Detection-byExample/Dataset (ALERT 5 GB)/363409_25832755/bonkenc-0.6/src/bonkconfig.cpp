...
configureBonkEnc::configureBonkEnc(bonkEncConfig *config)
{
	...
	btn_cancel		= new SMOOTHButton("Cancel", NIL, pos, size, SMOOTHProc(configureBonkEnc, this, Cancel));
	btn_cancel->SetOrientation(OR_LOWERRIGHT);

	pos.x -= 88;

	btn_ok			= new SMOOTHButton("OK", NIL, pos, size, SMOOTHProc(configureBonkEnc, this, OK));
	...
	group_mode			= new SMOOTHGroupBox("Encoder mode", pos, size);

	pos.x += 176;

	group_stereo			= new SMOOTHGroupBox("Stereo mode", pos, size);
	...
	check_lossless			= new SMOOTHCheckBox("Enable lossless encoding", pos, size, &lossless, SMOOTHProc(configureBonkEnc, this, SetEncoderMode));

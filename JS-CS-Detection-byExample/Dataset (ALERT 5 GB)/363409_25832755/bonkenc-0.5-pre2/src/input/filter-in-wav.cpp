...
FilterWAV::FilterWAV(bonkEncConfig *config) : InputFilter(config)
{
	setup = false;
...
bool FilterWAV::EncodeData(unsigned char **data, int size, int *outsize)
{
	*outsize = size;
...
bool FilterWAV::DecodeData(unsigned char **data, int size, int *outsize)
{
	*outsize = size;
...
bonkFormatInfo FilterWAV::GetAudioFormat()
{
	return format;
}

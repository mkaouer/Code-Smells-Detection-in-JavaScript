...
FilterAU::FilterAU(bonkEncConfig *config) : InputFilter(config)
{
	setup = false;
...
bool FilterAU::EncodeData(unsigned char **data, int size, int *outsize)
{
	*outsize = size;
...
bool FilterAU::DecodeData(unsigned char **data, int size, int *outsize)
{
	*outsize = size;
...
bonkFormatInfo FilterAU::GetAudioFormat()
{
	return format;
}

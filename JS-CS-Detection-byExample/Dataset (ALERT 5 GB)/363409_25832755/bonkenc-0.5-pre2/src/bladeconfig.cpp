...
configureBladeEnc::configureBladeEnc(bonkEncConfig *config)
{
	...
	btn_cancel		= new SMOOTHButton("Cancel", NIL, pos, size, SMOOTHProc(configureBladeEnc, this, Cancel));
	btn_cancel->SetOrientation(OR_LOWERRIGHT);

	pos.x -= 88;

	btn_ok			= new SMOOTHButton("OK", NIL, pos, size, SMOOTHProc(configureBladeEnc, this, OK));
	...
	group_dualchannel		= new SMOOTHGroupBox("Channels", pos, size);

	pos.x += 176;

	group_private			= new SMOOTHGroupBox("Private bit", pos, size);
	...
	check_dualchannel		= new SMOOTHCheckBox("Dual channel encoding", pos, size, &dualchannel, NULLPROC);

	pos.x += 176;

	check_private			= new SMOOTHCheckBox("Set Private bit", pos, size, &priv, NULLPROC);

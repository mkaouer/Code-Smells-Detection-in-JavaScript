...
//            return GraphicsUtilities.createThumbnailFast(src, size); // TODO uses bilinear - necessary? slow?
//        }
...
//          System.out.println("ImageUtils.createSoftTemplateBrush softness = " + softness);
//          AppLogic.debugImage(brushImage);

        return brushImage;
    }

    public static void drawGrid(Graphics2D g, int maxX, int maxY, int hWidth, int hSpacing, int vWidth, int vSpacing) {
        if (hWidth < 0) {
    ...
    public static void drawBrickGrid(Graphics2D g, int size, int maxX, int maxY) {
        if (size < 1) {

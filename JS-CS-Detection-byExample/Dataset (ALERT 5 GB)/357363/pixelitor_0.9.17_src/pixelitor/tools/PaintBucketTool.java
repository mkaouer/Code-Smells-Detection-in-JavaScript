 ...
 * A paint bucket tool.
 */
public class PaintBucketTool extends ForwardingTool {
    private RangeParam toleranceParam = new RangeParam("Tolerance", 0, 255, 0);
    private JComboBox fillComboBox;

    public PaintBucketTool() {
        super('p', "Paint Bucket", "paint_bucket_tool_icon.gif", "click to fill with the foreground color", Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
    }

    @Override
    void initSettingsPanel(ToolSettingsPanel p) {
        p.add(new SliderSpinner(toleranceParam, false, SliderSpinner.TextPosition.WEST));
        p.add(new JLabel("Fill With:"));

        fillComboBox = new JComboBox(new Object[]{FillType.FOREGROUND, FillType.BACKGROUND, FillType.TRANSPARENT});
        p.add(fillComboBox);
    }
...
//            AppLogic.debugImage(workingCopy);
            ...
            AffectedArea affectedArea = new AffectedArea(comp, replacedArea, true);
...
//        System.out.println("PaintBucketTool.scanlineFloodFill newRGB = " + newRGB);
        ...
        Deque<Point> stack = new ArrayDeque<Point>(); // the double-ended queue is used as a simple LIFO stack

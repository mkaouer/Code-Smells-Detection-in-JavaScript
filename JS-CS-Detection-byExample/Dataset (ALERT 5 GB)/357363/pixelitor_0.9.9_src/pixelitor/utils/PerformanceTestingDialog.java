...
public class PerformanceTestingDialog extends JDialog implements ActionListener, PropertyChangeListener {
    ...
    public PerformanceTestingDialog(Frame parent) {
        ...
        opSelector = new JComboBox(Operations.getAllOperationsSorted());
        opSelector.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                Operation op = (Operation) opSelector.getSelectedItem();
                if (op instanceof OperationWithGUI) {
                    OperationWithGUI dialogOp = (OperationWithGUI) op;
        ...
        public Void doInBackground() {
            ...
            for (int i = 0; i < executions; i++) {
                ...
                if (isCancelled()) {
                    ...
                    String results = getReport(op.getMenuName(), (i + 1), totalTime);
                    showResults(results);
            ...
            totalTime = (System.nanoTime() - startTime) / 1000000;
            String results = getReport(op.getMenuName(), executions, totalTime);
            showResults(results);

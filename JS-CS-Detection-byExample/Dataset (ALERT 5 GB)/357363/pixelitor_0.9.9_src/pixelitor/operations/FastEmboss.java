...
public class FastEmboss extends Operation {
    ...
    public BufferedImage transform(BufferedImage src, BufferedImage dest) {
        ...
        for (int y = 0; y < height - 1; y++) {
            destPixels[(y * width)] = GRAY_AS_32BIT_INT; // first column
            destPixels[(width - 1) + y * width] = GRAY_AS_32BIT_INT; // last column
        }

        for (int x = 0; x < width; x++) {
            destPixels[x] = GRAY_AS_32BIT_INT; // first row
...
//                    destPixels[x + y * width] = (currentAlpha | red << 16 | green << 8 | blue);
        ...
        if (!ImageUtils.hasPackedIntArray(src)) {
            ...
            g.drawImage(image, 0, 0, currentImageComponent);
            g.dispose();
        }

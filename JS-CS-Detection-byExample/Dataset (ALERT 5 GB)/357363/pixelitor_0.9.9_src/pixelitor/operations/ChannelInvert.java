 ...
 */
public class ChannelInvert extends OperationWithParametrizedGUI {
    ...
    IntChoiceParam.Value[] invertChoices = new IntChoiceParam.Value[]  {
    ...
    private IntChoiceParam invertTypeSelector = new IntChoiceParam("Invert Channel", invertChoices);

    public ChannelInvert() {
        super("Channel Invert", true);
        paramSet = new ParamSet(invertTypeSelector);
    }

    @Override
    public BufferedImage transform(BufferedImage src, BufferedImage dest) {
        int invertType = invertTypeSelector.getCurrentInt();
    ...
    private static BufferedImage invertHSB(int invertType, BufferedImage src, BufferedImage dest) {
        ...
        for (int i = 0; i < destData.length; i++) {
            ...
            switch (invertType) {
                case HUE_ONLY:         newRGB = Color.HSBtoRGB(0.5f + hsb[0], hsb[1],        hsb[2]);
                break;
                case BRI_ONLY:         newRGB = Color.HSBtoRGB(hsb[0],        hsb[1],        1.0f - hsb[2]);
                break;
                case SATURATION_ONLY:  newRGB = Color.HSBtoRGB(hsb[0],        1.0f - hsb[1], hsb[2]);
                break;
                case HUE_BRI:          newRGB = Color.HSBtoRGB(0.5f + hsb[0], hsb[1],        1.0f - hsb[2]);
                break;
                case HUE_SAT:          newRGB = Color.HSBtoRGB(0.5f + hsb[0], 1.0f - hsb[1], hsb[2]);
                break;
                case SAT_BRI:          newRGB = Color.HSBtoRGB( hsb[0],       1.0f - hsb[1], 1.0f - hsb[2]);
                break;
                case HUE_SAT_BRI:      newRGB = Color.HSBtoRGB(0.5f + hsb[0], 1.0f - hsb[1], 1.0f - hsb[2]);
                break;
            }

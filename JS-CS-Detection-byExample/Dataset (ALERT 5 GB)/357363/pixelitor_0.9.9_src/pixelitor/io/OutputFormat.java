...
public enum OutputFormat {
    JPG {
        ...
        public void doSaveComposition(Composition comp, File file) {
            BufferedImage img = comp.getCompositeImage("OutputFormat.doSaveComposition");
            BufferedImage finalImage = ImageUtils.convertToRGB(img); // no alpha support, convert first to RGB
            OpenSaveManager.saveImageToFile(file, finalImage, toString());
        }
    }, PNG {
        @Override
        public void doSaveComposition(Composition comp, File file) {
            BufferedImage finalImage = comp.getCompositeImage("OutputFormat.doSaveComposition"); // the format supports alpha, no need to convert ARGB to RGB
            OpenSaveManager.saveImageToFile(file, finalImage, toString());
        }
    }, GIF {
        @Override
        ...
        @Override
        public void doSaveComposition(Composition comp, File file) {
            BufferedImage compositeImage = comp.getCompositeImage("OutputFormat.doSaveComposition");
            BufferedImage finalImage = ImageUtils.convertToRGB(compositeImage); // no alpha support, convert first to RGB
            OpenSaveManager.saveImageToFile(file, finalImage, toString());
        }
    }, PXC {
        @Override

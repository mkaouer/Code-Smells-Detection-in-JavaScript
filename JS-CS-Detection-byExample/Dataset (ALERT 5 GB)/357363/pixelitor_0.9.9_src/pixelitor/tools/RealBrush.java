...
public class RealBrush implements Brush {
    ...
    public RealBrush() {
        templateImage = createTemplateBrush(25, 0.2);
    }

    /**
     * This method assumes that the color of coloredBrushImage is OK
     */
    private void resizeBrushImage(int newSize, boolean force) {
        if(!force) {
        ...
        finalScaledImage = new BufferedImage(newSize, newSize, BufferedImage.TYPE_INT_ARGB);
        Graphics2D g = finalScaledImage.createGraphics();
        g.drawImage(coloredBrushImage, 0, 0, newSize, newSize, null);
        g.dispose();
    }
    ...
    private void updateStateBeforeDrawing(Graphics2D g, int diameter) {
        Color c = g.getColor();
    ...
    @Override
    public void drawLine(Graphics2D g, int startX, int startY, int endX, int endY, int diameter) {
        updateStateBeforeDrawing(g, diameter);
        ...
        int halfSize = diameter / 2;

        for (int i = 0; i < delta; i++) {
            int interpolatedX = (int) x;
            int interpolatedY = (int) y;

            drawPointNoCheck(g, interpolatedX - halfSize, interpolatedY - halfSize);
     ...
     * brush image are white
     */
    private static BufferedImage createTemplateBrush(int radius, double density) {
            ...
            for (int y = 0; y < size; y++) {
                ...
                if (centerDistance2 < radius2) {
                    if (density > Math.random()) {
                        ...
                        pixels[x + y * size] = 0xFFFFFFFF;  // white
                    }
                } else {
                    pixels[x + y * size] = 0xFFFFFFFF; // white
                }

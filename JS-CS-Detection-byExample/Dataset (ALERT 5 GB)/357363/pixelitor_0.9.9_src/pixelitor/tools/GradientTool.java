...
public class GradientTool extends Tool {
    ...
    public void initSettingsPanel(JPanel p) {
        ...
        colorTypeSelector = new JComboBox(ColorType.values());
...
//        p.add(opacitySlider);
    ...
    public static void drawGradient(ImageLayer layer, Type type, ColorType colorType, MultipleGradientPaint.CycleMethod cycleMethod, Composite composite, Point start, Point end, boolean invert) {
        if (start.equals(end)) {
            return;
        }
        BufferedImage drawingImage = layer.createTmpDrawingImage(composite);

        int translationX = -layer.getTranslationX();
        int translationY = -layer.getTranslationY();
        start.translate(translationX, translationY);
        end.translate(translationX, translationY);

...
public class OpenSaveManager {
    ...
    public static void openFile(final File selectedFile) {
        Runnable r = new Runnable() {
            ...
            public void run() {
                ...
                PixelitorWindow.getInstance().addNewImage(img, selectedFile.getName());
    ...
    public static int save(ImageComponent activeImageComponent) {
        ...
        if (status == JFileChooser.APPROVE_OPTION) {
            ...
            saveFile(selectedFile, activeImageComponent.getCompositeImage(), outputFormat);
    ...
    public static void saveFile(final File selectedFile, final BufferedImage bufferedImage, final OutputFormat outputFormat) {
        ...
        Runnable r = new Runnable() {
            ...
            public void run() {
                try {
                    BufferedImage finalImage = outputFormat.getFinalImage(bufferedImage);
                    ImageIO.write(finalImage, outputFormat.toString(), selectedFile);
                } catch (IOException e) {
    ...
    public static void saveImageInAllFormats(BufferedImage bufferedImage, File saveDir) {
        OutputFormat[] outputFormats = OutputFormat.values();
        for (OutputFormat outputFormat : outputFormats) {
            File f = new File(saveDir, "all_formats." + outputFormat.toString());
            saveFile(f, bufferedImage, outputFormat);
        }
    }
    ...
    public static void openAllImagesInDir(File dir) {
        java.io.FileFilter imageFilter = new java.io.FileFilter() {
            ...
            public boolean accept(File f) {
                return Utils.isSupportedExtension(f.getName(), SUPPORTED_INPUT_FORMATS);
            }
        };

...
public class FilterCreator extends JPanel {
    ...
    public String createFilterSource() {
        ...
        addTransform(sb, pixelLoop, proxy, proxyName, center, edge, interpolation);
    ...
    private static void addTransform(StringBuilder sb, boolean pixelLoop, boolean jhProxy, String proxyName, boolean center, boolean edgeAction, boolean interpolation) {
        sb.append("\n    @Override\n");
        sb.append("    public BufferedImage transform(BufferedImage src, BufferedImage dest, ImageChangeReason changeReason) {\n");
        if (pixelLoop) {
    ...
    private static void addImports(StringBuilder sb, String name, boolean pixelLoop, boolean parametrizedGUI, boolean proxy, String proxyName) {
        sb.append("import pixelitor.ImageChangeReason;\n");
        if (pixelLoop) {
            sb.append("import pixelitor.utils.ImageUtils;\n");
        }
        sb.append("import pixelitor.operations.gui.ParamSet;\n");
        sb.append("import pixelitor.operations.gui.RangeParam;\n");
        if (parametrizedGUI) {
            sb.append("import pixelitor.operations.OperationWithParametrizedGUI;\n");
        }
    ...
    private static void addParamSetToConstructor(StringBuilder sb, ParameterInfo... params) {
        if (params.length == 1) {
            sb.append("        paramSet = new ParamSet(" + params[0].getVariableName() + ");\n");
        } else {
    ...
    private static void addParamsDeclaration(StringBuilder sb, boolean center, boolean edge, boolean color, boolean interpolation, ParameterInfo... params) {
        ...
        for (ParameterInfo param : params) {
            String paramName = param.getVariableName();
            sb.append("    private RangeParam " + paramName + " = new RangeParam(\"" + param.getName() + "\", " + param.getMin() + ", " + param.getMax() + ", " + param.getDefaultValue() + ");");

            sb.append("\n");
        }

        if (center) {
            sb.append("    private ImagePositionParam center = new ImagePositionParam(\"Center\");\n");
        }
        if (edge) {
            sb.append("    private IntChoiceParam edgeAction =  IntChoiceParam.getEdgeActionChoices();\n");
        }
        if (interpolation) {
            sb.append("    private IntChoiceParam interpolation = IntChoiceParam.getInterpolationChoices();\n");
        }
        if (color) {
            sb.append("    private ColorParam color = new ColorParam(\"Color:\", Color.WHITE);\n");
        }

...
atrun -t run -s "..\bin\at_server%EX%.exe -f cfg\at_server.cfg -t" -d . -c "ruby Runner.rb localhost %2 definitions\server\fix40\*.def definitions\server\fix41\*.def definitions\server\fix42\*.def" -i .\

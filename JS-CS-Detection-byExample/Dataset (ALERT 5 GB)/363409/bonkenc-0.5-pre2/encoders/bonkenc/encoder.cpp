...
void BONKencoder::begin(OutStream *_f_out, const char *text, uint32 _length, uint32 _rate, int _channels, bool _lossless, bool _mid_side, int _n_taps, int _down_sampling, int _samples_per_packet, double _quant_level)
{
	f_out			= _f_out;
	channels		= _channels;
	rate			= _rate;
	...
	info_pos = f_out->GetPos();

	for (int i = 0; i < (length + samples_size - 1) / samples_size; i++)
	{
...
void BONKencoder::store_packet(vector<int> &samples)
{
	...
	int	 c_pos = f_out->GetPos();

	f_out->Seek(info_pos);

	if (bit_out.bit_no == 0)
	{
		f_out->OutputNumber(c_pos - data_pos, 4);
		f_out->OutputNumber(8, 1);
	}
	else
	{
		f_out->OutputNumber(c_pos - data_pos + 1, 4);
		f_out->OutputNumber(bit_out.bit_no, 1);
	}

...
FilterVOC::FilterVOC(bonkEncConfig *config) : InputFilter(config)
{
	setup = false;
	bytesleft = 0;
...
bool FilterVOC::EncodeData(unsigned char **data, int size, int *outsize)
{
	*outsize = size;
...
bool FilterVOC::DecodeData(unsigned char **data, int size, int *outsize)
{
	*outsize = size;
...
bonkFormatInfo FilterVOC::GetAudioFormat()
{
	return format;
}

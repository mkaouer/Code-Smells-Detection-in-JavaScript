...
configureGeneralSettings::configureGeneralSettings(bonkEncConfig *config)
{
	...
	bonkVersion.Append("v").Append(bonk_get_version_string());
	...
	lameVersion.Append("v").Append(get_lame_short_version());

	encoders_combo_encoder->AddEntry(SMOOTHString("LAME ").Append(lameVersion), NULLPROC);

	SMOOTHString	 vorbisVersion = "";

	vorbisVersion.Append("v").Append("1.0rc3");

	encoders_combo_encoder->AddEntry(SMOOTHString("OggVorbis ").Append(vorbisVersion), NULLPROC);
#endif
...
configureGeneralSettings::~configureGeneralSettings()
{
	...
	reg_register->UnregisterObject(register_layer_cdrip);
...
SMOOTHVoid configureGeneralSettings::OK()
{
	...
	currentConfig->cdrip_activedrive = cdrip_combo_drive->GetSelectedEntry();

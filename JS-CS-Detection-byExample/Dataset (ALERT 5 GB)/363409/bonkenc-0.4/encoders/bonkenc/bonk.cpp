...
void modified_levinson_durbin(vector<int> &x, 
  ...
  for(int i=0;i<k_out.size();i++) {
    ...
    if (k  > 1024/tap_quant[i]) k =   1024/tap_quant[i];
    if (-k > 1024/tap_quant[i]) k = -(1024/tap_quant[i]);
...
void BONKencoder::begin(OutStream *_f_out, const char *text, uint32 length, uint32 _rate, int _channels, bool _lossless, bool _mid_side, int _n_taps, int _down_sampling, int _samples_per_packet, double _quant_level)
{
	f_out			= _f_out;
	channels		= _channels;
	rate			= _rate;
...
void BONKencoder::store_packet(vector<int> &samples)
{
	...
	if (!lossless)
	{
		...
		for(int channel=0;channel<channels;channel++) 
			for(int i=0;i<samples_per_packet;i++)
				energy += double(output_samples[channel][i]) * output_samples[channel][i];

		energy = sqrt(energy/(channels*samples_per_packet));

		quant = int(quant_level*energy/sample_factor);

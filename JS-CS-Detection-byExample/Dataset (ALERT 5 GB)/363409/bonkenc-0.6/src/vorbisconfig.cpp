...
configureVorbisEnc::configureVorbisEnc(bonkEncConfig *config)
{
	...
	mainWnd			= new SMOOTHWindow("Ogg Vorbis encoder configuration");
	...
	btn_cancel		= new SMOOTHButton("Cancel", NIL, pos, size, SMOOTHProc(configureVorbisEnc, this, Cancel));
	btn_cancel->SetOrientation(OR_LOWERRIGHT);

	pos.x -= 88;

	btn_ok			= new SMOOTHButton("OK", NIL, pos, size, SMOOTHProc(configureVorbisEnc, this, OK));
	...
	group_mode			= new SMOOTHGroupBox("Encoding Mode", pos, size);

	size.cy = 84;

	group_mode2			= new SMOOTHGroupBox("Encoding Mode", pos, size);
	...
	option_mode_vbr			= new SMOOTHOptionBox("VBR (Variable Bitrate)", pos, size, &mode, 0, SMOOTHProc(configureVorbisEnc, this, SetMode));

	pos.x += 166;

	option_mode_abr			= new SMOOTHOptionBox("ABR (Average Bitrate)", pos, size, &mode, 1, SMOOTHProc(configureVorbisEnc, this, SetMode));

	pos.x = 19;
	pos.y += 43;

	text_quality			= new SMOOTHText("Quality:", pos);
	...
	text_abr			= new SMOOTHText("Average Bitrate:", pos);
